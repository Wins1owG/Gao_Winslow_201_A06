...
[https://github.com/Wins1owG/Gao_Winslow_201_A06/tree/main](https://github.com/Wins1owG/Gao_Winslow_201_A06/tree/main)
...
for (player in names(player_info)) {
  ppg <- player_info[[player]]$points / player_info[[player]]$games_played
...
```{r}
rounded_ppg <- lapply(points_per_game, round, 2)
rounded_ppg <- unlist(rounded_ppg)
print(rounded_ppg)
```
...
mat_round <- apply(mat_rand, 1:2, round_func)
print(mat_round)
```
...
```{r}
average_temp <- apply(max_temps[, colnames(max_temps)[3:14]], 2, mean, na.rm = TRUE)
print(average_temp)
```
...
```{r}
max_temps_CO <- subset(max_temps, state == "CO")

max_temps_OH <- subset(max_temps, state == "OH")

max_temps_VA <- subset(max_temps, state == "VA")
```
...
```{r}
max_temps_CO$annual_median <- apply(max_temps_CO[, 3:14], 1, median)
max_temp_year <- max_temps_CO$year[which.max(max_temps_CO$annual_median)]
max_temp_value <- max(max_temps_CO$annual_median)
min_temp_year <- max_temps_CO$year[which.min(max_temps_CO$annual_median)]
min_temp_value <- min(max_temps_CO$annual_median)
print(sprintf("The maximum median annual temperature of %.2f occurred in %d and the minimum median annual temperature of %.2f occurred in %d.",max_temp_value, max_temp_year, min_temp_value, min_temp_year))
```
